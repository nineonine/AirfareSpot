...
// get credentials to access aws
const authenticateViaFB = function authenticateViaFB(token) {
  return new Promise((resolve, reject) => {
    AWS.config.credentials = new AWS.CognitoIdentityCredentials({
      ...
      LocalStorage.setItem('awsCredentials', JSON.stringify(awsCredentials));
      console.log("authenticated!: " + awsCredentials)
      resolve();
    });
  })
}

const fetchEmailFromFB = function fetchEmailFromFB() {
  ...
  return new Promise((resolve, reject) => {
    const _responseInfoCallback = function(error, result) {
      if (error) {
        ...
        console.log("got email: " + result.email)
        resolve(result.email)
      }
    }
    const infoRequest = new GraphRequest(
      '/me',
      { httpMethod: 'GET', version: 'v2.5',
        parameters: {
            'fields': {
                'string' : 'email'
            }
...
const userExists = async function userExists(email) {
  ...
  const returnedUsers = await new Promise ((resolve, reject) => {
...
function handleSignOut() {
  ...
  LocalStorage.removeItem('usCategories');
